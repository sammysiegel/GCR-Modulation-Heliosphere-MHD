...
int main(int argc, char **argv)
{
   ...
   else if (mpi_config->is_worker)
   {
      ...
      data_file.open("diffusion_data_new_old.txt");
         ...
         for (j = 0; j < 1; j++)
         {
            for (k = 0; k < N; k++)
            {
               ...
               else
               {
                    ...
                    del_b = sqrt(spdata.region[del_b_index])/21.81;

                    if (spdata.Uvec.Norm() > 3.0) {
                        in_HS = 0;
                        kappa_para = ((3 * GSL_CONST_CGSM_SPEED_OF_LIGHT * GSL_CONST_CGSM_SPEED_OF_LIGHT * vel[0] * vel[0] * vel[0] * unit_velocity_fluid * unit_velocity_fluid * unit_velocity_fluid * GSL_CONST_CGSM_MASS_PROTON * GSL_CONST_CGSM_MASS_PROTON)
                                / (20 * SPC_CONST_CGSM_ELECTRON_CHARGE * SPC_CONST_CGSM_ELECTRON_CHARGE * sin(3 * M_PI / 5))
                                * (1 + (72/7)*pow(
                                    (l_perp * unit_length_fluid * SPC_CONST_CGSM_ELECTRON_CHARGE * spdata.Bmag * unit_magnetic_fluid)/(GSL_CONST_CGSM_MASS_PROTON * vel[0] * unit_velocity_fluid * GSL_CONST_CGSM_SPEED_OF_LIGHT), 1.667
                                ))
                                / (l_perp * unit_length_fluid * del_b*del_b*unit_magnetic_server*unit_magnetic_server)); //* (- 0.4 * tanh((pos.Norm()-15)/5) + 0.6);
...
//                         << std::setw(15) << spdata.Uvec.Norm()
//                         << std::setw(15) << spdata.Bmag
//                         << std::setw(15) << spdata.region[0]
//                         << std::setw(15) << spdata.region[1]
//                         << std::setw(15) << l_perp
//                         << std::setw(15) << del_b
                         << std::setw(15) << kappa_old
                         << std::setw(15) << kappa_para
                         << std::setw(15) << kappa_perp
                         << std::endl;
            };
         };
      };
