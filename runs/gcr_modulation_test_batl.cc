...
// Initial position
   GeoVector init_pos(1.5 * one_au, 0.0, 0.0);
...
// Radius
   double inner_boundary = 1.1 * one_au;
...
// Pass ownership of "diffusion" to simulation
   ...
   simulation->AddDiffusion(DiffusionSammy(), container);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   simulation->MainLoop();
   simulation->PrintDistro1D(0, 0, "output/modulated_gcr_spectrum_SSSW_fixednewKdriftsNLGC_10au.dat", true);
   simulation->PrintDistro1D(1, 0, "output/time_gcr_trajec_SSSW_fixednewKdriftsNLGC_10au.dat", true);
