...
// Lower bound for momentum
   double momentum1 = Mom(5 * SPC_CONST_CGSM_MEGA_ELECTRON_VOLT / unit_energy_particle, specie);
...
// Radius
   double outer_boundary = 2.0 * one_au;
...
// Max duration of the trajectory
   double maxtime = -60.0 * 60.0 * 24.0 * 3.0 * 1.0 / unit_time_fluid;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   simulation->PrintRecords(0, "trajectory_X.lines", true);
   return 0;
};
